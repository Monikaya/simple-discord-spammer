...
async def remfriend(user, token, broxy):
    headers = {
        'accept': '*/*',
    ...
    try:
        res = await broxy.delete(f"https://discord.com/api/v9/users/@me/relationships/{user}", headers=headers, timeout=100)
        return res
...
async def startpp(user, token, proxy):
    async with AsyncClient(proxies={'https://': 'http://' + proxy}) as broxy:
        res = await remfriend(user, token, broxy)
...
async def getinfo():
    ...
    user = input("What userid would you like to remove? ")

    async with TaskPool(2_00) as pool:
        for token in tokens:
            await pool.put(startpp(user, token, proxyprocess.GetProxy()))
    input("Done removing friend requests. Press enter to exit! ")
    exit()
